...
# ## Isosplit
# ### Jeremy Magland 2015, Apache v2.0 license
...
"""
python version of labeller
"""
__author__ = "Jeremy Magland, port: Koustubh Sudarshan, Alex Karavos, Siamak Salavatian, Olujimi Ajijola, Guy Kember"
__version__ = "1.0"
...
# In[13]:
#multi_iso_split main
parser = argparse.ArgumentParser(description='Variable mapping: default value')
...
parser.add_argument("--f", default=50, type=int, help="f: max_passes: def=50")
...
if split_factor >= min_cluster_size :
    print('Exception: split_factor exceeds min_cluster_size ',split_factor, min_cluster_size)
    sys.exit()
#check bounds
...
#get labels
labels_parcelate = parcelate(X, min_parcel_size, max_parcels, split_factor)
if main_verbose: print('unique labels ',np.unique(labels_parcelate))
 
for isocut_threshold in np.linspace(0.1, 1.0, 10) :

    #init
    pass_iterations = 0    
    final_pass = False   
    
    #deep copy labels_parcelate
    labels = np.copy(labels_parcelate)
    ...
    while 1 :
        ...
        #if final_pass: done
        if final_pass or pass_iterations > max_passes : 
            break
    ...
    #del dependency vars
    del clusters_changed_vec_in_pass
    del data_comparisons_made
    del clusters_changed
